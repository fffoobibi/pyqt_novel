...
class MoreWidget(QWidget, MoreUi):  # 阅读设置界面
    ...
    @property
    def max_pics(self):  # 最大背景图片记录
        return 25

    def setBackgroundPic(self, picture: str = None, reload: bool=True):  # 设置阅读背景图片
        if picture:
            file = picture.replace('\\', '/')
        else:
            home_dir = QDir(
    ...
    load_local_pic = partialmethod(setBackgroundPic, reload=False)  # 加载背景资源文件
    ...
    def load_local_sources(self): # 加载本地资源
        abs_path = os.path.abspath('./fonts/bkgs')
        join = os.path.join
        for file in os.listdir(abs_path):
            self.load_local_pic(join(abs_path, file))
